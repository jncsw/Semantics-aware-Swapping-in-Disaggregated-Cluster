...
//   ptedit_use_implementation(PTEDIT_IMPL_KERNEL);
  ...
  ptedit_entry_t vm = ptedit_resolve(address, 0);
    ptedit_entry_t vm2 = ptedit_resolve(target, 0);
  if(vm.pgd == 0) {
  ...
  printf(TAG_PROGRESS "address[0] = " COLOR_YELLOW "%c" COLOR_RESET"\n", *(volatile char*)address);
    printf(TAG_PROGRESS "target[0] = " COLOR_YELLOW "%c" COLOR_RESET"\n", *(volatile char*)target);
  ...
  vm.pte = ptedit_set_pfn(vm.pte, target_pfn);
    vm2.pte = ptedit_set_pfn(vm.pte, address_pfn);
  // update only PTE
  vm.valid = PTEDIT_VALID_MASK_PTE;
  ptedit_update(address, 0, &vm);

    vm2.valid = PTEDIT_VALID_MASK_PTE;
    ptedit_update(target, 0, &vm2);

  printf(TAG_OK "address[0] = " COLOR_YELLOW "%c" COLOR_RESET "\n", *(volatile char*)address);


    printf(TAG_OK "target[0] = " COLOR_YELLOW "%c" COLOR_RESET "\n", *(volatile char*)target);
