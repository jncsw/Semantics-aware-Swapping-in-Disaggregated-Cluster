...
//	int type = 0x1;
	unsigned long request_addr 	= ARRAY_START_ADDR; // 4GB, start of RDMA meta space, 1GB not exceed the swap partitio size.
	unsigned long size  		= ARRAY_BYTE_SIZE;	// 1GB, for unsigned long, length is 0x4,000,000
	size_t array_slice = size / sizeof(unsigned long)
